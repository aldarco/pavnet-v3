...
if args[0] in ["-h", "h", "--help", "help"]:
    print("HELP:") 
    print(" required arguments: <folder> <prefix [PLO,PIU, ...]>")
    print(" optional arguments (after required): <separator [',', 'auto', ...]> <engine ['python', 'C', ...]>")
    sys.exit(0)
    
# Gral params
# must
folder1 =  args.pop(0) # "/data/pavnet/2025/PIURA/09042025/" # contains subfolders
LOC = args.pop(0)      # location PIU , PLO

# Optional params
sep = args.pop(0) if args else ","
sep = None if sep=="auto" else sep
engine = args.pop(0) if args else "c"
...
for subfolder in subfolders[:]:
    ...
    print("Subfolder:", path_, end=" -> content: ")
    ...
    for k, f in enumerate(fnames[:]):
        ...
        [st[:,0] , st[:,1]] = st[:,0], st[:,0] #utils.IQ_clipping_filter(st)
...
print(f"Total processing time: {time.time()-ti} s | Files = {countfiles}")
print("Saving file...")
...
fileoutputname = f"/{resfolder}/{LOC}_AMPLITUDE_{ampdf0.index[0].date()}-to-{ampdf0.index[-1].date()}.csv"

ampdf0.to_csv(fileoutputname,index=True)
